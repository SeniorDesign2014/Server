...
func (this *Client) Send(c appengine.Context, pn *PushNotification) (resp *PushNotificationResponse) {
	...
	err = this.ConnectAndWrite(c, resp, payload)
	if err != nil {
		resp.Success = false
		resp.Error = err
		return
...
func (this *Client) ConnectAndWrite(c appengine.Context,resp *PushNotificationResponse, payload []byte) (err error) {
	...
	conn, err := socket.Dial(c, "tcp", this.Gateway)
	if err != nil {
		return err
	}
	defer conn.Close()
